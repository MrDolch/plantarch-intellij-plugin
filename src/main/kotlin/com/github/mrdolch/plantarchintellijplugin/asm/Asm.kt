...
data class Parameters(
    val title: String?,
    val caption: String?,
    val classesToAnalyze: Set<String> = emptySet(),
    val classesToHide: Set<String> = emptySet(),
    val markerClasses: MarkerClasses = MarkerClasses(emptySet()),
    val librariesToHide: Set<String> = emptySet(),
    val libraryPaths: Set<Path>,
    val classPaths: Set<Path>,
    val showLibraries: Boolean = false,
    val showPackages: ShowPackages = ShowPackages.NONE,
    val showUseByMethodNames: UseByMethodNames = UseByMethodNames.NONE,
    val targetPumlFile: String,
    val plantumlInlineOptions: String,
    val dependencyTypes: Set<DepKind>,
)

data class MarkerClasses(val values: Set<String>) {
...
object Asm {
  ...
  fun renderDiagram(parameters: Parameters): Result {
    ...
    val promotedJars: Set<Path> =
        findJarsContainingClasses(parameters.libraryPaths, parameters.classesToAnalyze)

    val effectiveRoots = parameters.classPaths.toMutableSet().apply { addAll(promotedJars) }

    val classDeps = scanClassPath(effectiveRoots)
    return toPlantUml(classDeps, parameters, promotedJars.map { it.fileName.toString() }.toSet())
  }

  private fun findJarsContainingClasses(
      libraryPaths: Set<Path>,
      classesToAnalyze: Set<String>,
  ): Set<Path> {
    if (classesToAnalyze.isEmpty()) return emptySet()
    val wanted = classesToAnalyze.map { it.replace('.', '/') + ".class" }.toSet()
    val result = mutableSetOf<Path>()
    libraryPaths.forEach { p ->
      if (p.toString().endsWith(".jar")) {
        JarFile(p.toFile()).use { jar ->
          val hasAny = jar.entries().asSequence().any { e -> e.name in wanted }
          if (hasAny) result.add(p)
        }
      }
    }
    return result
  ...
  fun toPlantUml(deps: Set<ClassDeps>, job: Parameters, promotedJarNames: Set<String>): Result {
    ...
    val focusContainers = klassenInFocus.map { it.container }.toSet()

    fun allowContainer(c: Container): Boolean {
      if (c == Containers.UNKNOWN) return false
      val isJar = c.name.endsWith(".jar")
      // Erlaubt, wenn kein Jar ODER wenn Jar promoted ist (per Name) ODER wenn Container ein
      // Fokus-Container ist
      return !isJar || promotedJarNames.contains(c.name) || focusContainers.contains(c)
    }

    val visibleEdges =
        deps
            .flatMap { it.deps }
            .filter { job.dependencyTypes.contains(it.kind) }
    ...
    val klassenInDiagram =
        (klassenInFocus +
                visibleEdges
                    ...
                    .map { it.from }
                    .filter { allowContainer(it.container) } +
                visibleEdges
        ...
        .map { klasse ->
          val marker =
              (edgesToMarkers.filter { it.from == klasse }.map { "<<${it.to.simplename}>>" } +
                      job.markerClasses.getStereotypes(klasse).map { "<<$it>>" })
                  .sorted()
                  .distinct()
                  .joinToString("")
